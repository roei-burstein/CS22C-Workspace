...
/**
 * A class that implements the ADT list by using a chain of DOUBLY linked nodes
 * that has a head dummy reference and tail dummy reference.
 * 
...
public class LList2<T> implements ListInterface<T> {
	...
	{
		Node2 currNode;
		currNode = firstDummyNode.next;

		while (!currNode.equals(lastDummyNode)) {
			if (currNode.getData() == anEntry) {
				Node2 nodeToRemove = currNode;
